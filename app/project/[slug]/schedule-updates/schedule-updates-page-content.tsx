...
interface ScheduleUpdatesPageContentProps {
  project: Project;
}

export default function ScheduleUpdatesPageContent({ project }: ScheduleUpdatesPageContentProps) {
  ...
  useEffect(() => {
    fetchUpdates();
  }, [project.slug, statusFilter, confidenceFilter]);
  ...
  const fetchUpdates = async () => {
    try {
      ...
      const response = await fetch(`/api/projects/${project.slug}/schedule-updates?${params.toString()}`);
  ...
  const handleApprove = async (updateId: string) => {
    try {
      setProcessing(true);
      const response = await fetch(`/api/projects/${project.slug}/schedule-updates/${updateId}`, {
        method: 'PATCH',
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      setProcessing(true);
      const response = await fetch(`/api/projects/${project.slug}/schedule-updates/${updateToReject}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      setProcessing(true);
      const response = await fetch(`/api/projects/${project.slug}/schedule-updates/bulk-action`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Link
                href={`/project/${project.slug}/schedules`}
