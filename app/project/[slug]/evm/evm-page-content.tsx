...
interface EVMPageContentProps {
  project: Project;
}

export default function EVMPageContent({ project }: EVMPageContentProps) {
  ...
  const fetchBudget = async () => {
    try {
      const res = await fetch(`/api/projects/${project.slug}/budget`);
      if (res.ok) {
  ...
  const handleSyncBudget = async () => {
    try {
      ...
      const res = await fetch(`/api/projects/${project.slug}/budget/sync`, {
        method: 'POST',
