...
import { X, AlertCircle, Flag, ArrowLeft, Loader2, Pin } from 'lucide-react';
...
interface AnnotationsPageContentProps {
  project: Project;
}

export default function AnnotationsPageContent({ project }: AnnotationsPageContentProps) {
  ...
  const fetchAnnotations = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${project.slug}/annotations?documentId=${selectedDoc.id}`
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCreateAnnotation = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/projects/${project.slug}/annotations`, {
        method: 'POST',
  ...
  const handleUpdateStatus = async (annotationId: string, newStatus: string) => {
    try {
      const res = await fetch(
        `/api/projects/${project.slug}/annotations/${annotationId}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
