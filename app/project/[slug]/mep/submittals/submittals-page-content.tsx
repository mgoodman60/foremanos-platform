...
export function SubmittalsPageContent({ projectSlug }: { projectSlug: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-6 pt-4">
        ...
        <ScheduleExtractionButton projectSlug={projectSlug} />
      </div>
      ...
      {/* Tab Content */}
      {activeTab === 'list' ? (
        <SubmittalList projectSlug={projectSlug} />
      ) : activeTab === 'dashboard' ? (
        <div className="px-6 pb-6">
          <BulkVerificationDashboard projectSlug={projectSlug} />
        </div>
      ) : activeTab === 'specs' ? (
        <div className="px-6 pb-6">
          <SpecSectionBrowser projectSlug={projectSlug} />
        </div>
      ) : (
        <div className="px-6 pb-6">
          <ToleranceSettings projectSlug={projectSlug} />
        </div>
      )}
    </div>
  );
