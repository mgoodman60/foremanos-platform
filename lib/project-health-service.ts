...
export async function calculateProjectHealth(projectId: string): Promise<HealthScoreResult> {
  ...
  if (reportsWithIncidents.length === 0 && dailyReports.length > 0) {
    // All clear for the period
    incidentsFree = 7;
  } else if (reportsWithIncidents.length > 0) {
...
export async function saveHealthSnapshot(projectId: string): Promise<void> {
  ...
  await prisma.projectHealthSnapshot.create({
    data: {
      ...
      scheduleScore: health.scheduleScore,
      budgetScore: health.budgetScore,
      safetyScore: health.safetyScore,
      qualityScore: health.qualityScore,
      documentScore: health.documentScore,
